...
```
Commitando e adicionando uma mensagem para o commit, é de suma importancia que todos os commits tenham uma descrição, assim dá para saber o que foi commitado e é o padrão.
...
```javascript
  git log  
```
histórico de commits, podemos ver todos os commits que já foram gerados, e a descrição, além também do codigo hash do Commit

(Ex. de Hash: 5687fds677fsd6fd8g7d9s789sd789sd7f)
#

```javascript
  git reflog
```
hitórico mais detalhado das alterações
...
```
Responsável por enviar as alterações do local para o github
...
#
## :keyboard: Comandos importantes para o Terminal
...
Git pull
```
irá puxar as informações do remoto e colocar dentro do repositório local

#
  :star: Dica: Apertar <kbd>.</kbd> No github abre o vscode online (Quando se está dentro do arquivo)
...
```
Irá trocar da branch atual para uma nova branch que está sendo criada
#
```javascript
